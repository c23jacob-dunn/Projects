...
typedef struct wordListStruct {
    ...
    bool test;      //True when running test cases
} WordList;
...
/**
 * @brief fills the listOfWords struct with the words from the file
 * @param listOfWords - WordList pointer
 * @param fileName - name of the file being used
 * @return none
 */
void loadWordList(WordList *listOfWords, char fileName[]);
